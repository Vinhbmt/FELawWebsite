...
const ViewMeetingUser = () => {
    ...
    const handlePayment = async () => {
        const response = await dispatch(await AccountUserAction.asyncProcessPayment({"destination": meetingInfo.lawyerId._id, "meetingId": meetingInfo._id, "amount": meetingInfo.price, "description": paymentContent}))
        console.log(response);
        if(response.status == 201 && response.data.isSuccess == true) {
            socket.emit("notification", {
                userId: meetingInfo.lawyerId._id,
                content: `${meetingInfo.userId.firstName + " " + meetingInfo.userId.lastName} đã thanh toán cuộc hẹn`,
    ...
    const handleUpdateDoneMeeting = async () => {
        const response = await dispatch(await LawyerAction.asyncUpdateAppointment({"meetingId": meetingInfo._id, "status": 3, "price": meetingInfo.price}));
        console.log(response);
        if(response.status == 200) {
            socket.emit("notification", {
                userId: meetingInfo.lawyerId._id,
                content: `${meetingInfo.userId.firstName + " " + meetingInfo.userId.lastName} đã hoàn thành cuộc hẹn`,
                url: `http://localhost:3000/lawyer/appointment/${meetingInfo._id}`,
              });
    ...
    const handleCallVideo = () => {
        ...
        callUser(meetingInfo.lawyerId.socketId, meetingInfo.userId.socketId);
    } 
        ...
        <>
            ...
            <div className="detail-meeting-container">
                ...
                <div className="detail-meeting">
                    <div className="detail-meeting-image" onClick={() => navigate(`/lawyer/${meetingInfo.lawyerId._id}`)}>
                    ...
                    <div className="detail-meeting-info">
                        <div className="detail-meeting-info1">
                            ...
                            <div onClick={() => navigate(`/lawyer/${meetingInfo.lawyerId._id}`)}>{meetingInfo.lawyerId.firstName + " " + meetingInfo.lawyerId.lastName}</div>
                        </div>
                        <div className="detail-meeting-info1">
                        ...
                        <>
                            ...
                            <div className="detail-meeting-info12">
                                <button className="btn-success" onClick={handleUpdateDoneMeeting}>Đánh dấu là hoàn thành</button>
                            </div>
                            <div className="detail-meeting-info1">
                ...
                }
                {
                    ...
                    <div className="meeting-status">
                        <button className="meeting-status-btn" onClick={handleViewPayment}>Thanh toán</button>
                    </div>
                    
                }
                {
                    meetingInfo.status == 2 &&
                    <div className="meeting-status">Đã thanh toán cuộc hẹn<img src="https://img.icons8.com/office/16/000000/checked--v1.png"/></div>
                }

            <Modal show={showPaymentModal} enforceFocus={false} className="modal-min modal-alert">
                <Modal.Header>
                    <Modal.Title>Thanh toán</Modal.Title>
                    <button className="btn btn-light" onClick={handleCloseViewPayment}></button>
                </Modal.Header>
                <Modal.Body>
                    <div className="payment-body">
                        <div className="payment-body1">
                            <div>Tới luật sư: </div>
                            <span>{meetingInfo.lawyerId.firstName + " " + meetingInfo.lawyerId.lastName}</span>
                        </div>
                        <div className="payment-body1">
                            <div>Số tiền: </div>
                ...
                <Modal.Footer>
                    ...
                    <button className="btn btn-default" onClick={handleCloseViewPayment}>Cancel</button>
                </Modal.Footer>
            </Modal>
        </>
    )
